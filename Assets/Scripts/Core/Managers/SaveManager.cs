...
public class SaveManager : Singleton<SaveManager>
{
    ...
    public void Save()
    {
        ...
        try
        {
            ...
            Data.upgrades.Flush();
            if (Data.characterStats != null) Data.characterStats.Flush();
        ...
        if (File.Exists(_savePath))
        {
            try
            {
                ...
                if (_data != null)
                {
                    ...
                    if (_data.characterStats == null) _data.characterStats = new CharacterStatsDictionary();
    ...
    // ── 难度解锁/整数存档 ──
    private static readonly Dictionary<string, int> _intStorage = new Dictionary<string, int>();

    public int GetPermanentInt(string key)
    {
        if (_intStorage.TryGetValue(key, out int val)) return val;
        // 从 upgrades 字典读取（复用现有存储）
        string strVal = PermanentUpgradeStore.GetPassiveBonus(Data, $"int_{key}").ToString();
        if (int.TryParse(strVal, out int parsed)) { _intStorage[key] = parsed; return parsed; }
        return 0;
    }

    public void SetPermanentInt(string key, int value)
    {
        _intStorage[key] = value;
        PermanentUpgradeStore.AddPermanentBonus(Data, $"int_{key}", value);
    }
    ...
    public CharacterSaveData GetCharacterStats(string characterId)
    {
        if (Data.characterStats == null) Data.characterStats = new CharacterStatsDictionary();
        return Data.characterStats.Get(characterId);
    }

    public void UpdateCharacterStats(string characterId, int wave, int kills)
    {
        if (Data.characterStats == null) Data.characterStats = new CharacterStatsDictionary();
        var stats = Data.characterStats.GetOrCreate(characterId);
        stats.totalGames++;
