...
/// <summary>
/// 角色统计可序列化字典 — 键为 characterId，值为 CharacterSaveData
/// </summary>
[Serializable]
public class CharacterStatsDictionary
{
    public string[] keys = new string[0];
    public CharacterSaveData[] values = new CharacterSaveData[0];

    [NonSerialized] private Dictionary<string, CharacterSaveData> _dict;
    ...
    {
        _dict = new Dictionary<string, CharacterSaveData>(keys.Length);
        for (int i = 0; i < keys.Length; i++)
            _dict[keys[i]] = values[i] ?? new CharacterSaveData();
    }

    private void SyncToArrays()
    {
        keys = new string[_dict.Count];
        values = new CharacterSaveData[_dict.Count];
        int idx = 0;
    ...
    public CharacterSaveData Get(string key)
    {
        EnsureBuilt();
        return _dict.TryGetValue(key, out var v) ? v : null;
    }

    public CharacterSaveData GetOrCreate(string key)
    {
        EnsureBuilt();
        if (!_dict.TryGetValue(key, out var v))
        {
            v = new CharacterSaveData();
            _dict[key] = v;
            _dirty = true;
        }
        return v;
...
public class SaveGameData
{
    ...
    // ── 角色独立统计 ──
    public CharacterStatsDictionary characterStats;
}
