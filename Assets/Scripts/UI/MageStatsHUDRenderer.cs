...
public static class MageStatsHUDRenderer
{
    ...
    /// 绘制左下角简化版统计条
    /// </summary>
    public static void DrawCompactBar(MagePassive mage, float marginLeft, float marginBottom,
        ...
        GUIStyle smallStyle)
    {
        ...
        // DPS
        float totalDps = MageStatsDataCollector.CalculateCurrentDPS(mage);
        ...
        // DOT 子弹图标条
        DrawDotGunIcons(mage, x + 8, y + 20, w - 16, dotTextures, smallStyle);
    ...
    /// 简化版底部的 DOT 子弹小图标条
    /// </summary>
    private static void DrawDotGunIcons(MagePassive mage, float x, float y, float maxW,
    ...
    /// 绘制暂停时的完整统计面板
    /// </summary>
    public static void DrawFullPanel(MagePassive mage, float marginLeft, float marginBottom,
        ...
        GUIStyle smallStyle, GUIStyle titleStyle, GUIStyle milestoneLabelStyle)
    {
        ...
        if (guns != null)
        {
            for (int i = 0; i < guns.Count; i++)
            {
                cy = DrawDotGunRow(x + 8, cy, panelW - 16, guns[i], mage,
        ...
        cy = DrawStatRow(x + 8, cy, panelW - 16, "Crit Chance", $"{mage.GetDotCritChance() * 100:F1}%",
            Color.Lerp(TextPrimary, Color.red, 0.3f), lineHeight, labelStyle, valueStyle);
        cy = DrawStatRow(x + 8, cy, panelW - 16, "Crit Multiplier", $"{mage.GetDotCritMultiplier():F1}x", TextPrimary, lineHeight, labelStyle, valueStyle);
        cy = DrawStatRow(x + 8, cy, panelW - 16, "DOT Duration +", $"{(mage.GetDotDurationMultiplier() - 1f) * 100:F0}%", TextPrimary, lineHeight, labelStyle, valueStyle);
    ...
    private static float DrawDotGunRow(float x, float y, float w, MagePassive.DotGunState gun,
        MagePassive mage, float lineHeight, Texture2D[] dotTextures,
        GUIStyle labelStyle, GUIStyle valueStyle, GUIStyle smallStyle)
    {
        ...
        GUI.color = TextSecondary;
        float effectiveCd = gun.cooldown * mage.GetAttackSpeedMultiplier();
