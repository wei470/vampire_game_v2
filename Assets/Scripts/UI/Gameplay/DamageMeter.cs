...
public class DamageMeter : MonoBehaviour
{
    ...
    private void OnDamageReceived(GameObject target, float damage, Vector3 sourcePos)
    {
    ...
    /// 记录一次伤害
    /// </summary>
    public void RecordDamage(DamageSource source, float damage, bool isCrit = false)
    {
    ...
    /// 记录引爆伤害
    /// </summary>
    public void RecordDetonate(float damage, int enemiesHit)
    {
        RecordDamage(DamageSource.Detonate, damage);
        // #41 追踪最高单次引爆伤害
        if (damage > _maxSingleDetonateDamage)
            _maxSingleDetonateDamage = (long)damage;
    }

    /// <summary>
    /// 记录 DOT tick 伤害
    /// </summary>
    public void RecordDotDamage(StatusEffectType dotType, float damage)
    {
        ...
        _totalDotTicks++;
        _damageSinceLastDpsCalc += (long)damage;
    }
    ...
    private DamageSource InferDamageSource(GameObject target, float damage)
    {
        // 检查目标是否有 DOT 效果
