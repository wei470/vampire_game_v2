...
{
    private static GUIStyle _notifIconStyle;
    private static GUIStyle _notifNameStyle;
    private static GUIStyle _notifDescStyle;
    private static GUIStyle _notifBonusStyle;
    ...
    private static void EnsureStyles()
    {
        ...
        _stylesInit = true;
        _notifIconStyle = new GUIStyle(GUI.skin.label) { fontSize = 28, alignment = TextAnchor.MiddleCenter };
        _notifNameStyle = new GUIStyle(GUI.skin.label) { fontSize = 18, fontStyle = FontStyle.Bold };
        _notifDescStyle = new GUIStyle(GUI.skin.label) { fontSize = 14 };
        _notifBonusStyle = new GUIStyle(GUI.skin.label) { fontSize = 13, fontStyle = FontStyle.Italic };
    }
    ...
    public static void DrawNotificationCard(NotificationEntry n, Texture2D notifBgTex, Texture2D notifBorderTex)
    {
        ...
        _notifIconStyle.normal.textColor = new Color(1f, 0.85f, 0.2f, alpha);
        GUI.Label(new Rect(x + 10, y + 10, 40, 40), "\U0001f3c6", _notifIconStyle);

        _notifNameStyle.normal.textColor = new Color(1f, 0.95f, 0.6f, alpha);
        GUI.Label(new Rect(x + 55, y + 8, NOTIF_WIDTH - 70, 24), n.name, _notifNameStyle);

        _notifDescStyle.normal.textColor = new Color(0.7f, 0.8f, 0.9f, alpha);
        GUI.Label(new Rect(x + 55, y + 32, NOTIF_WIDTH - 70, 20), n.description, _notifDescStyle);

        if (!string.IsNullOrEmpty(n.bonusText))
        {
            _notifBonusStyle.normal.textColor = new Color(0.3f, 1f, 0.5f, alpha);
            GUI.Label(new Rect(x + 55, y + 52, NOTIF_WIDTH - 70, 18), $"\u2728 {n.bonusText}", _notifBonusStyle);
        }
