...
public class AchievementUI : MonoBehaviour
{
    ...
    private static readonly Dictionary<string, float> _tempBonusSummary = new Dictionary<string, float>(8);

    private GUIStyle _achieveTitleStyle;
    private GUIStyle _achieveBonusTitleStyle;
    private GUIStyle _achieveBonusLineStyle;
    private GUIStyle _achieveBonusTotalStyle;
    private GUIStyle _achieveStatStyle;
    ...
    private void EnsureStyles()
    {
        ...
        _stylesInit = true;
        _achieveTitleStyle = new GUIStyle(GUI.skin.label) { fontSize = 28, alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, normal = { textColor = UIColorTheme.AccentCyan } };
        _achieveBonusTitleStyle = new GUIStyle(GUI.skin.label) { fontSize = 15, fontStyle = FontStyle.Bold, normal = { textColor = new Color(0.2f, 1f, 0.4f) } };
        _achieveBonusLineStyle = new GUIStyle(GUI.skin.label) { fontSize = 12, normal = { textColor = new Color(0.8f, 0.9f, 0.7f) } };
        _achieveBonusTotalStyle = new GUIStyle(GUI.skin.label) { fontSize = 13, fontStyle = FontStyle.Bold, normal = { textColor = new Color(0.3f, 1f, 0.6f) } };
        _achieveStatStyle = new GUIStyle(GUI.skin.label) { fontSize = 18, alignment = TextAnchor.MiddleCenter };
    }
    ...
    {
        _tempBonusSummary.Clear();
        foreach (var a in _achievements)
        {
            if (!a.unlocked || string.IsNullOrEmpty(a.bonusKey) || a.bonusValue <= 0f) continue;
            if (!_tempBonusSummary.ContainsKey(a.bonusKey))
                _tempBonusSummary[a.bonusKey] = 0f;
            _tempBonusSummary[a.bonusKey] += a.bonusValue;
        }
        return _tempBonusSummary;
    }
    ...
    private void DrawAchievementPanel()
    {
        ...
        var titleStyle = _achieveTitleStyle;
        ...
        var bonusTitleStyle = _achieveBonusTitleStyle;
        ...
        float bx = x + 15;
        float by = bonusY + 22;
        var bonusLineStyle = _achieveBonusLineStyle;
        ...
        {
            // 总加成显示
            var totalStyle = _achieveBonusTotalStyle;
        ...
        // 统计和关闭按钮
        var statStyle = _achieveStatStyle;
