...
public class WaveChallengeSystem : MonoBehaviour
{
    ...
    public void DrawChallengeUI()
    {
        ...
        GUI.Label(new Rect(x, y + 15, w, 35), "⚔️ 波次挑战 ⚔️", _titleStyle);

        GUI.Label(new Rect(x, y + 55, w, 28), _currentChallenge.title, _nameStyle);

        GUI.Label(new Rect(x + 20, y + 90, w - 40, 40), _currentChallenge.description, _descStyle);

        GUI.Label(new Rect(x, y + 135, w, 22), "✨ " + _currentChallenge.rewardText, _rewardStyle);
        ...
        GUI.Label(acceptRect, "⚔️ 接受挑战", _btnStyle);
        ...
        GUI.Label(declineRect, "🏠 安全通过", _btnStyle);
    ...
    private void EnsureStyles()
    {
        ...
        _titleStyle = new GUIStyle(GUI.skin.label) { fontSize = 24, fontStyle = FontStyle.Bold, alignment = TextAnchor.MiddleCenter, normal = { textColor = new Color(1f, 0.85f, 0.2f) } };
        _nameStyle = new GUIStyle(GUI.skin.label) { fontSize = 20, fontStyle = FontStyle.Bold, alignment = TextAnchor.MiddleCenter, normal = { textColor = new Color(1f, 0.4f, 0.3f) } };
        _descStyle = new GUIStyle(GUI.skin.label) { fontSize = 15, alignment = TextAnchor.MiddleCenter, wordWrap = true, normal = { textColor = new Color(0.8f, 0.9f, 1f) } };
        _rewardStyle = new GUIStyle(GUI.skin.label) { fontSize = 14, alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Italic, normal = { textColor = new Color(0.3f, 1f, 0.5f) } };
        _btnStyle = new GUIStyle(GUI.skin.label) { fontSize = 17, fontStyle = FontStyle.Bold, alignment = TextAnchor.MiddleCenter, normal = { textColor = Color.white } };
    }
