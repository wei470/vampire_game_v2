...
/// </summary>
[CreateAssetMenu(fileName = "DotBulletConfig", menuName = "Mage/Dot Bullet Config")]
public class DotBulletConfig : ScriptableObject
{
    // ══════════════════════════════════════════════════════════════
    // 燃烧子弹（BurnBullet）
    ...
    [Tooltip("暗影 DOT 死亡传播的基础效率（小数形式）。\n" +
             ...
             "默认值：0.5（即 50%）")]
    public float DarkBaseEfficiency = 0.5f;
    ...
    // ── 单例访问 ──
    private static DotBulletConfig _instance;

    /// <summary>
    /// 获取全局配置实例（从 Resources 加载）
    /// 如果没有找到配置文件，返回默认值的临时实例
    /// </summary>
    public static DotBulletConfig GetDefault()
    {
        if (_instance == null)
        {
            _instance = Resources.Load<DotBulletConfig>("Configs/DotBulletConfig");
            if (_instance == null)
            {
                // 回退：创建临时实例使用默认值
                _instance = CreateInstance<DotBulletConfig>();
                DebugHelper.LogWarning("[DotBulletConfig] 未找到 Resources/Configs/DotBulletConfig.asset，使用默认值");
            }
        }
        return _instance;
    }
