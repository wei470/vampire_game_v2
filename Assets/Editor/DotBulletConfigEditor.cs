...
/// <summary>
/// DotBulletConfig 可视化配置编辑器。
/// 以表格形式展示所有子弹参数，支持 DPS 曲线预览。
///
/// 打开方式：菜单 Mage → Dot Bullet Config Editor
/// </summary>
public class DotBulletConfigEditor : EditorWindow
{
    private DotBulletConfig _config;
    ...
    {
        var window = GetWindow<DotBulletConfigEditor>("DOT 子弹配置");
    ...
    private void OnGUI()
    {
        ...
        _config = (DotBulletConfig)EditorGUILayout.ObjectField(
            "DotBulletConfig", _config, typeof(DotBulletConfig), false);
        if (EditorGUI.EndChangeCheck())
            RebuildDpsCurve();

        if (_config == null)
        {
            EditorGUILayout.HelpBox("请拖入 DotBulletConfig ScriptableObject，或点击下方按钮自动查找。", MessageType.Info);
            if (GUILayout.Button("自动查找"))
            {
                var guids = AssetDatabase.FindAssets("t:DotBulletConfig");
                if (guids.Length > 0)
                {
                    _config = AssetDatabase.LoadAssetAtPath<DotBulletConfig>(
